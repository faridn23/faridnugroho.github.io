  ...
  function favTeam(result) {
    ...
    if (data.length == 0) {
      favteams += `
        ...
        <h3 style="magin-top: 30px; display: inline-block; width:100%;" class="center-align">Kamu belum memilih Tim favoritmu</h3>
        `
      ...
      favteams += `
        ...
        <ul class="collapsible" id="list-fav-teams"></ul>
        `
      data.forEach(data => {
        listTeams += `
          <li>
            <div class="collapsible-header">
              <img src="${data.crestUrl}" alt="${data.name}" width="50" height="50" style="margin-right: 30px;">
              <span style="display: inline-block; margin-top: 20px;">${data.name}</span>
            </div>
            <div class="collapsible-body">
              <div class="detail-team">
                ...
                <div id="delete" style="margin-top: 50px; margin-left: 30px;">
                  <a class="btn-floating btn-large pulse red">
                    <i class="material-icons delete" data-id="${data.id}" data-name="${data.name}">delete</i>
                  </a>
                </div>
              </div>
